...
class Disasm_command(commands.Cog):
    ...
    @app_commands.command(name="disasm", description="Disassembles attachement or URL provided")
    ...
    async def disassembleFile(self, interaction: discord.Interaction, attachment: typing.Optional[discord.Attachment], url: typing.Optional[str]): # typing.Union doesn't seem to work for this unfortunately
        if isinstance(attachment, discord.Attachment):
            downloadedFile = await dl.download(interaction.namespace.attachment.url, str(interaction.user.id), self.bot)
            ...
            try:
                downloadedFile = await dl.download(url, str(interaction.user.id), self.bot)
                disassembled_file = await disasm.processFile(downloadedFile)
            ...
            await interaction.response.send_message("Please provide a file url or attachement")
